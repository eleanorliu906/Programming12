...
void buildPannel() {
  ...
  if (ogTowerButton.clicked) {
    if (money >= 10) towers.add(new TowerOriginal(mouseX, mouseY, 0, 30));
    else if (money <= 10) texts.add(new Text(width/2, height/2, "Not enough money"));
  } else if (circleTowerButton.clicked) {
    if (money >= 10) towers.add(new TowerCircle(mouseX, mouseY, 0, 30));
    else if (money <= 10) texts.add(new Text(width/2, height/2, "Not enough money"));
  } else if (sniperTowerButton.clicked) {
    if (money >= 10) towers.add(new TowerSniper(mouseX, mouseY, 0, 30));
    else if (money <= 10) texts.add(new Text(width/2, height/2, "Not enough money"));
  }
}
