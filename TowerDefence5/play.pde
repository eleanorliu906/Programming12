...
void resetTowers() {
  for (int i = 0; i < towers.size(); i++) {
    ...
    t.cooldown = 0;
    if (t instanceof TowerSniper || t instanceof TowerSniper2) t.sniper = false;
  }
}
...
void addMobs() {
  ...
  float r = random(1);
  if (r >  0.2) { // hide back the ghost mob
    mobs.add(new MobGhost(50, -40*waveCount));
    texts.add(new Text(50, 20, 15, "Beware of Ghost Mob!", 15*waveCount));
  }
  waveCount++;
}
