  ...
  void act() {
    ...
    if (mobs.size() > 0) {
      ...
      if (cooldown % threshold*0.5 == 0 && !excessBombs()) {
        powerups.add(new PowerUpBomb(x, y, 30 + 30*level*level));
        cooldown = 0;
      }
    }
    ...
    for (int i = 0; i < mobs.size(); i++){
      Mob m = mobs.get(i);
      // myX > t.x - 0.5*t.size && myX < t.x + 0.5*t.size && myY > t.x - 0.5*t.size && myY < t.y + 0.5*t.size
      if (x > m.x - 0.5*m.w && x < m.x + 0.5*m.w && y > 0.5* m.h && y < 0.5*m.h){
        m.hp--;
      }
    }
  }
