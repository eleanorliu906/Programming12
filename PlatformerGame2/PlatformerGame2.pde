...
PImage ice, stone, treetrunk, middletree, lefttree, righttree, normaltree, spike, bridge, bridgeright, bridgeleft, bridgetop, bridgetopleft, bridgetopright, trampoline;
PImage[] lava = new PImage[6];
int lavaIndex;
PImage heart;
PImage[] idle = new PImage[2];
PImage[] jump = new PImage[1];
PImage[] runleft = new PImage[3];
PImage[] runright = new PImage[3];
  ...
  for (int j = 0; j <= map.height; j++) {
    for (int i = 0; i <= map.width; i++) {
      ...
      if (c == black) { // stone block
        ...
        if (s == treeTrunkBrown) {
          createBlocks(i, j, "tree", middletree, 4, 0.2, false);
        } else if ( e == treeGreen && w == treeGreen) {
          createBlocks(i, j, "tree", normaltree, 4, 0.2, false);
        } else if (w == white && e == treeGreen) {
          createBlocks(i, j, "tree", lefttree, 4, 0.2, false);
        } else if (e == white && w == treeGreen) {
          createBlocks(i, j, "tree", righttree, 4, 0.2, false);
        }
...
void loadImages() {
  ...
  runright[0] = loadImage("runright0.png");
  runright[1] = loadImage("runright1.png");
  runright[2] = loadImage("runright2.png");
