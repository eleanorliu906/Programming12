...
PImage ice, stone, treetrunk, middletree, lefttree, righttree, normaltree, spike, bridge, bridgetop, trampoline;
  ...
  for (int j = 0; j <= map.height; j++) {
    for (int i = 0; i <= map.width; i++) {
      ...
      if (c == black) { // stone block
        ...
        FBridge br = new FBridge(i*gridSize, j*gridSize, bridge);
        terrain.add(br);
        world.add(br);
      } else if (c == red) {
        ...
        FBridge br = new FBridge(i*gridSize, j*gridSize, bridgetop);
        terrain.add(br);
        world.add(br);
      }
    }
